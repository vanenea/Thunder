...
public class GoodsServiceImpl implements GoodsService {
	...
	public Goods getGoodsInfor() {
		...
		try {
			...
			skuQuantity = JSON.parseObject(skuQuantity.getString("4248070790759"));
			...
			if(goods.getSkuQuantity() > 0 && "1".equals(goods.getIsSold()) && Double.parseDouble(goods.getPrice())<=59.00 ) {
				...
				commonUtil.sendMail("商品通知", text);
			}
			
		} catch (Exception e) {
			LOGGER.info("#### 查询商品失败 ######", e );
			String text = "您查询商品程序出错啦,请去日志文件中查看原因 !!! </br> <code>"+ e.getLocalizedMessage() +"</code>";
			commonUtil.sendMail("程序错误通知", text);
